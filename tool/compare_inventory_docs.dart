...
Future<void> main() async {
  ...
  hajiDocs.sort((a, b) => (a['id'] as String).compareTo(b['id'] as String));
  print('=== ALL ${hajiDocs.length} inventory_haji DOC IDs ===');
  for (final d in hajiDocs) {
    print('${d['id']} | Qty: ${d['quantity']} | Exp: ${d['expiryDate']} | Batch: ${d['batchNumber']} | Barcode: ${d['barcode']} | CreatedAt: ${d['createdAt']}');
  }

  print('\n=== SADDAR DOC IDs ===');
  final saddarDocs = await fetchAllDocs('branches/karachi/inventory_saddar');
  saddarDocs.sort((a, b) => (a['id'] as String).compareTo(b['id'] as String));
  for (final d in saddarDocs) {
    print('${d['id']} | Qty: ${d['quantity']} | Exp: ${d['expiryDate']} | Batch: ${d['batchNumber']} | Barcode: ${d['barcode']}');
  }
}
