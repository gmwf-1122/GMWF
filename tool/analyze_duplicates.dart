...
void analyze(String colName, List<Map<String, dynamic>> docs) {
  print('========================================');
  print('ANALYSIS FOR $colName (Total docs: ${docs.length})');
  print('========================================');

  // Group by (normalized name, normalized type, normalized dose)
  ...
  for (final entry in groups.entries) {
    if (entry.value.length > 1) {
      duplicateGroups++;
      redundantDocs += (entry.value.length - 1);
      print('\n[DUPLICATE GROUP] ${entry.key}:');
      for (final doc in entry.value) {
        print('  - ID: ${doc['id']}');
        print('    Name: "${doc['name']}", Dose: "${doc['dose']}", Type: "${doc['type']}", Qty: ${doc['quantity']}, Exp: ${doc['expiryDate']}, Created: ${doc['createdAt']}');
      }
    }
  }

  print('\nSummary for $colName: $duplicateGroups groups have duplicates (Total redundant extra docs: $redundantDocs)');
}

Future<void> main() async {
  final hajiDocs = await fetchAllDocs('branches/karachi/inventory_haji');
  analyze('branches/karachi/inventory_haji', hajiDocs);

  final saddarDocs = await fetchAllDocs('branches/karachi/inventory_saddar');
  analyze('branches/karachi/inventory_saddar', saddarDocs);
}
