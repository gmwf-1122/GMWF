...
class _ChairmanScreenState extends State<ChairmanScreen>
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          ...
          // FIX: ClipRect prevents any inner widget from painting into the
          // sidebar area, and Material gives DonationsScreen a fresh
          // Scaffold ancestor context so its TabBar doesn't escape.
          Expanded(
            child: ClipRect(
  ...
  Widget _buildBody() {
    switch (_pageIndex) {
      case -2:
        return _ChairmanDashboard(t: _t, username: widget.username);
      case -1:
        // FIX: Wrap in Material so DonationsScreen's internal Scaffold/TabBar
        // does not inherit or affect the outer Scaffold's tab controller.
        return Material(
          color: _t.bg,
          child: _AllBranchDonationsView(t: _t, username: widget.username),
        );
      case 0:
        return RoleThemeScope(role: RoleTheme.chairman, child: const Branches());
      case 1:
        return RoleThemeScope(role: RoleTheme.chairman, child: const Register());
      case 2:
        return RoleThemeScope(
            role: RoleTheme.chairman,
            child: Container(
                color: _t.bg,
  ...
  Widget _sidebar() => Container(
        ...
        child: Column(children: [
          ...
          _navTile(Icons.analytics_outlined, 'Overview', _pageIndex == -2,
              _goDashboard),
          _navTile(Icons.volunteer_activism_rounded, 'Donations',
              _pageIndex == -1, _goDonations,
              accentColor: _cGreen),
          ...
          Expanded(
            child: ListView(padding: EdgeInsets.zero, children: [
              _navTile(Icons.account_balance_outlined, 'Branches',
                  _pageIndex == 0, () => _go(0)),
              _navTile(Icons.person_add_outlined, 'Register User',
                  _pageIndex == 1, () => _go(1)),
              _navTile(Icons.favorite_border_rounded, 'Patients',
                  _pageIndex == 2, () => _go(2)),
              _navTile(Icons.people_outline_rounded, 'Users',
                  _pageIndex == 3, () => _go(3)),
              _navTile(Icons.download_outlined, 'Download',
                  _pageIndex == 4, () => _go(4)),
            ]),
          ),
          Divider(height: 1, color: _t.bgRule, indent: 24, endIndent: 24),
          const SizedBox(height: 4),
          _navTile(Icons.logout_rounded, 'Sign Out', false, _logout,
              danger: true),
          const SizedBox(height: 24),
        ]),
      );
  ...
  Widget _navTile(
    ...
    return Padding(
      ...
      child: Material(
        color: active
            ? (accentColor ?? _t.accent).withOpacity(0.10)
            : Colors.transparent,
...
// FIX: Each DonationsScreen.embedded() is wrapped in a Material widget.
//      This breaks the DefaultTabController.of(context) lookup chain — the
//      inner DonationsScreen finds its own TabController instead of
//      resolving to this widget's _tabController, which was causing inner
//      tab changes to update the branch selector instead of the
//      Dashboard/Credits tabs inside donations.
// ════════════════════════════════════════════════════════════════════════════
...
class _AllBranchDonationsView extends StatefulWidget {
  ...
  @override
  State<_AllBranchDonationsView> createState() =>
      _AllBranchDonationsViewState();
}
      ...
      Container(
        ...
        child: SafeArea(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 12),
            // FIX: Use explicit controller (not DefaultTabController) so
            // this TabBar is never confused with inner DonationsScreen tabs.
            TabBar(
      ...
      Expanded(
        child: TabBarView(
          ...
          children: _branches.map((b) {
            // FIX: Each branch tab is wrapped in a Material widget.
            // Material creates a new context boundary so that
            // DefaultTabController.of(context) inside DonationsScreen
            // does NOT resolve upward to _tabController (the branch
            // selector). Instead DonationsScreen creates its own
            // independent TabController for Dashboard/Credits/etc. tabs.
            return Material(
              color: t.bg,
              ...
              Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                        colors: [t.accent, t.accent.withOpacity(0)]),
