...
class PatientRegisterPageState extends State<PatientRegisterPage> {
  ...
  Future<void> _savePatient() async {
    ...
    try {
      ...
      // We do this BEFORE calling FirestoreService so that even if the
      // network call throws mid-way the patient is not lost.
      await LocalStorageService.saveLocalPatient(patientMap);
      debugPrint('[PatientRegister] ✅ Hive write: $patientId');

      // STEP 2 — FirestoreService handles: direct Firestore write (fast path)
      //           → queue for retry on failure → LAN broadcast → triggerUpload
